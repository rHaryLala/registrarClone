...
<body class="bg-gray-50">
    @if (view()->exists('teacher.components.sidebar'))
        @include('teacher.components.sidebar')
    @elseif (view()->exists('dean.components.sidebar'))
        @include('dean.components.sidebar')
    @endif

    <div class="main-content min-h-screen">
        @if (view()->exists('teacher.components.header'))
            @include('teacher.components.header')
        @elseif (view()->exists('dean.components.header'))
        ...
        <main class="p-4 md:p-6 lg:p-8">
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100" id="courseTableBody">
                        @forelse($courses as $course)
                                <tr class="cursor-pointer course-card hover:bg-blue-50/50 transition-all duration-300" onclick="window.location='{{ route('teacher.courses.show', $course->id) }}'"> 
                                <td class="px-6 py-5">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 bg-gradient-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center text-white font-semibold mr-4">{{ strtoupper(substr($course->sigle, 0, 2)) }}</div>
                                        <div><div class="text-sm font-semibold text-gray-900">{{ $course->sigle }}</div></div>
                                    </div>
                                </td>
                                <td class="px-6 py-5"><div class="text-sm font-medium text-gray-900">{{ $course->nom }}</div></td>
                                <td class="px-6 py-5"><span class="credit-badge inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">{{ $course->credits }} crédits</span></td>
                                <td class="px-6 py-5"><div class="text-sm text-gray-600">{{ $course->teacher->name ?? 'Non attribué' }}</div></td>
                                <td class="px-6 py-5"><span class="inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">{{ $course->categorie }}</span></td>
                            </tr>
                        @empty
            ...
            <div class="mobile-cards space-y-4" id="mobileCourseCards">
                @forelse($courses as $course)
                    <div class="course-card bg-white rounded-2xl p-6 shadow-lg border border-gray-100" data-sigle="{{ strtolower($course->sigle) }}" data-nom="{{ strtolower($course->nom) }}" data-mention="{{ strtolower($course->mentions && $course->mentions->count() ? $course->mentions->pluck('nom')->join(', ') : ($course->mention->nom ?? '')) }}">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center">
                                <div class="w-12 h-12 bg-gradient-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center text-white font-bold text-lg mr-4">{{ strtoupper(substr($course->sigle, 0, 2)) }}</div>
                                <div><h3 class="font-semibold text-gray-900 text-lg">{{ $course->sigle }}</h3><p class="text-gray-600 text-sm">{{ $course->nom }}</p></div>
                            </div>
                            <span class="credit-badge inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">{{ $course->credits }} crédits</span>
                        </div>
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div><p class="text-xs text-gray-500 uppercase tracking-wider">Professeur</p><p class="text-sm font-medium text-gray-900">{{ $course->teacher->name ?? 'Non attribué' }}</p></div>
                            <div><p class="text-xs text-gray-500 uppercase tracking-wider">Mention</p>@if($course->mention)<span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-purple-100 text-purple-800">@if(isset($course->mentions) && $course->mentions->count()){{ $course->mentions->pluck('nom')->join(', ') }}@else{{ $course->mention->nom ?? 'Mention non définie' }}@endif</span>@else<span class="text-gray-400 text-sm">-</span>@endif</div>
                            <div><p class="text-xs text-gray-500 uppercase tracking-wider">Catégorie</p><span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">{{ $course->categorie }}</span></div>
                        </div>
                    </div>
                @empty
        ...
        <main class="p-4 md:p-6 lg:p-8">
            ...
            <div class="search-container rounded-2xl p-6 mb-8 text-white">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    {{-- <a href="{{ route('teacher.courses.create') }}" class="bg-white text-blue-800 px-6 py-3 rounded-xl hover:bg-blue-50 transition-all duration-300 flex items-center justify-center md:justify-start font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-1">
