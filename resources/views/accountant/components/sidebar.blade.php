<div class="sidebar fixed inset-y-0 left-0 bg-blue-800 text-white z-50 shadow-2xl">
    ...
    <nav class="p-4 flex-1 overflow-y-auto">
        <ul class="space-y-1">
            <li>
                @php $accDashboardActive = request()->routeIs('accountant.dashboard'); @endphp
                <a href="{{ route('accountant.dashboard') }}"
                   class="nav-item group flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 hover:bg-blue-700/50 hover:translate-x-1 {{ $accDashboardActive ? 'bg-blue-900/80 text-blue-100 shadow-lg' : 'hover:text-blue-100' }}"
                   {{ $accDashboardActive ? 'aria-current="page"' : '' }}>
                    <div class="w-5 h-5 flex items-center justify-center">
                        <i class="fas fa-tachometer-alt text-sm group-hover:scale-110 transition-transform duration-200"></i>
                    </div>
                    <span class="font-medium">Tableau de bord</span>
                    <div class="ml-auto w-1 h-6 bg-blue-300 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 {{ $accDashboardActive ? 'opacity-100' : '' }}"></div>
                </a>
            </li>
        </ul>
