...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-blue-100 min-h-screen font-body">
    <?php echo $__env->make('multimedia.components.sidebar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    
    <div class="main-content min-h-screen">
        <?php echo $__env->make('multimedia.components.header', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            ...
            <div class="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                ...
                <div class="col-span-1 md:col-span-1 flex justify-center md:justify-start">
                    <div class="w-full md:w-80 lg:w-96 rounded-3xl overflow-hidden bg-white/10 backdrop-blur-sm shadow-2xl relative group">
                        <form id="photo-upload-form" action="<?php echo e(route('multimedia.students.update', $student->id)); ?>" method="POST" enctype="multipart/form-data" class="absolute inset-0 w-full h-full z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                            <?php echo csrf_field(); ?>
                        ...
                        <?php if($student->image): ?>
                            <img id="profile-photo" src="<?php echo e(asset($student->image)); ?>" alt="Photo de profil" class="object-cover w-full h-64 md:h-96">
                        <?php else: ?>
                            <div class="flex items-center justify-center w-full h-64 md:h-96 text-white font-bold text-6xl">
                                <span id="profile-initials"><?php echo e(substr($student->prenom, 0, 1)); ?><?php echo e(substr($student->nom, 0, 1)); ?></span>
                            </div>
                        <?php endif; ?>
                ...
                <div class="col-span-1 md:col-span-2 flex flex-col justify-between">
                    <div>
                        <h1 class="text-4xl md:text-5xl font-heading font-bold mb-4 text-balance bg-clip-text"><?php echo e($student->prenom); ?> <?php echo e($student->nom); ?></h1>
                        <div class="flex flex-wrap items-center gap-3 mb-4">
                            <span class="bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full text-sm font-semibold border border-white/20 transition-all duration-300">
                                <i class="fas fa-id-badge mr-2 text-white/70"></i>Matricule: <?php echo e($student->matricule); ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="info-item p-4 bg-white/5 rounded-lg">
                                ...
                                <div class="text-sm font-medium"><?php echo e($student->email ?? '-'); ?></div>
                            </div>
                            <div class="info-item p-4 bg-white/5 rounded-lg">
                                <div class="text-xs text-white/80 uppercase">Niveau</div>
                                <div class="text-sm font-medium"><?php echo e(optional($student->yearLevel)->label ?? '-'); ?></div>
                            </div>
                        </div>
                    </div>

                    <div class="mt-6 flex items-center justify-start space-x-4">
                        <a href="<?php echo e(route('multimedia.dashboard')); ?>" class="inline-flex items-center gap-3 px-5 py-3 rounded-2xl bg-white/10 hover:bg-white/15 transition text-white font-semibold">
...
.gradient-bg, .section-header {
    ...
    span.addEventListener('dblclick', function() {
        ...
        function save() {
            ...
            fetch("<?php echo e(route('multimedia.students.update', $student->id)); ?>", {
                method: 'POST',
                headers: {
    ...
    async function recomputeSemesterFee() {
        try {
            const res = await fetch("<?php echo e(url('/multimedia/students/' . $student->id . '/recompute-semester-fee')); ?>", {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'Accept': 'application/json',
...
<?php /**PATH D:\PROJET REGISTRAIRE\registrarClone\registrar\resources\views/multimedia/students/show.blade.php ENDPATH**/ ?>