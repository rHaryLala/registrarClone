...
<head>
    ...
    <title>Liste des cours - Enseignant</title>
    ...
    <style>
        ...
        .course-card { transition: all 0.3s ease; border-left: 4px solid transparent; }
        .course-card:hover { transform: translateY(-2px); box-shadow: 0 10px 25px rgba(0, 0, 0, 0.1); border-left-color: #1e40af; }
        .search-container { background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); }
        .credit-badge { transition: all 0.2s ease; }
        .credit-badge:hover { transform: scale(1.05); }
    ...
    <div class="main-content min-h-screen">
        ...
        <main class="p-4 md:p-6 lg:p-8">
            <div class="search-container rounded-2xl p-6 mb-8 text-white">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-blue-100">Vos cours</p>
                ...
                <div class="mt-6 flex flex-col md:flex-row gap-4">
                    <div class="flex-1">
                        <div class="relative">
                            <input type="text" id="searchInput" placeholder="Rechercher par sigle ou nom du cours..." class="w-full px-4 py-3 pl-12 rounded-xl text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-white/30">
                    ...
                    <thead class="bg-gradient-to-r from-blue-800 to-blue-700 text-white">
                        <tr>
                            <th class="px-6 py-4 text-left text-sm font-semibold uppercase tracking-wider" data-sort="sigle" data-direction="asc">
                                <div class="flex items-center gap-2"><i class="fas fa-code text-blue-200"></i> Sigle</div>
                            </th>
                            <th class="px-6 py-4 text-left text-sm font-semibold uppercase tracking-wider"><div class="flex items-center gap-2"><i class="fas fa-book text-blue-200"></i> Nom du cours</div></th>
                            <th class="px-6 py-4 text-left text-sm font-semibold uppercase tracking-wider"><div class="flex items-center gap-2"><i class="fas fa-star text-blue-200"></i> Crédits</div></th>
                            <th class="px-6 py-4 text-left text-sm font-semibold uppercase tracking-wider"><div class="flex items-center gap-2"><i class="fas fa-chalkboard-teacher text-blue-200"></i> Professeur</div></th>
                            <th class="px-6 py-4 text-left text-sm font-semibold uppercase tracking-wider"><div class="flex items-center gap-2"><i class="fas fa-layer-group text-blue-200"></i> Catégorie</div></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100" id="courseTableBody">
                        <?php $__empty_1 = true; $__currentLoopData = $courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="cursor-pointer course-card hover:bg-blue-50/50 transition-all duration-300" onclick="window.location='<?php echo e(route('teacher.courses.show', $course->id)); ?>'"> 
                                <td class="px-6 py-5">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 bg-gradient-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center text-white font-semibold mr-4"><?php echo e(strtoupper(substr($course->sigle, 0, 2))); ?></div>
                                        <div><div class="text-sm font-semibold text-gray-900"><?php echo e($course->sigle); ?></div></div>
                                    </div>
                                </td>
                                <td class="px-6 py-5"><div class="text-sm font-medium text-gray-900"><?php echo e($course->nom); ?></div></td>
                                <td class="px-6 py-5"><span class="credit-badge inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800"><?php echo e($course->credits); ?> crédits</span></td>
                                <td class="px-6 py-5"><div class="text-sm text-gray-600"><?php echo e($course->teacher->name ?? 'Non attribué'); ?></div></td>
                                <td class="px-6 py-5"><span class="inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800"><?php echo e($course->categorie); ?></span></td>
                            </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <tr><td colspan="7" class="px-6 py-12 text-center"><div class="text-gray-400"><i class="fas fa-book text-4xl mb-4"></i><p class="text-lg font-medium">Aucun cours trouvé</p><p class="text-sm">Contactez l'administrateur si vous pensez que c'est une erreur</p></div></td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div class="mobile-cards space-y-4" id="mobileCourseCards">
                <?php $__empty_1 = true; $__currentLoopData = $courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="course-card bg-white rounded-2xl p-6 shadow-lg border border-gray-100" data-sigle="<?php echo e(strtolower($course->sigle)); ?>" data-nom="<?php echo e(strtolower($course->nom)); ?>" data-mention="<?php echo e(strtolower($course->mentions && $course->mentions->count() ? $course->mentions->pluck('nom')->join(', ') : ($course->mention->nom ?? ''))); ?>">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center">
                                <div class="w-12 h-12 bg-gradient-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center text-white font-bold text-lg mr-4"><?php echo e(strtoupper(substr($course->sigle, 0, 2))); ?></div>
                                <div><h3 class="font-semibold text-gray-900 text-lg"><?php echo e($course->sigle); ?></h3><p class="text-gray-600 text-sm"><?php echo e($course->nom); ?></p></div>
                            </div>
                            <span class="credit-badge inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800"><?php echo e($course->credits); ?> crédits</span>
                        </div>
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div><p class="text-xs text-gray-500 uppercase tracking-wider">Professeur</p><p class="text-sm font-medium text-gray-900"><?php echo e($course->teacher->name ?? 'Non attribué'); ?></p></div>
                            <div><p class="text-xs text-gray-500 uppercase tracking-wider">Mention</p><?php if($course->mention): ?><span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-purple-100 text-purple-800"><?php if(isset($course->mentions) && $course->mentions->count()): ?><?php echo e($course->mentions->pluck('nom')->join(', ')); ?><?php else: ?><?php echo e($course->mention->nom ?? 'Mention non définie'); ?><?php endif; ?></span><?php else: ?><span class="text-gray-400 text-sm">-</span><?php endif; ?></div>
                            <div><p class="text-xs text-gray-500 uppercase tracking-wider">Catégorie</p><span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800"><?php echo e($course->categorie); ?></span></div>
                        </div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <div class="text-center py-12"><div class="text-gray-400"><i class="fas fa-book text-6xl mb-6"></i><h3 class="text-xl font-semibold mb-2">Aucun cours trouvé</h3><p class="text-gray-500">Commencez par contacter l'administrateur</p></div></div>
                <?php endif; ?>
            </div>
        </main>
    </div>
    ...
    <script>
        function filterCourses() {
            ...
            tableRows.forEach(row => {
                ...
                if (matchesSearch && matchesMention) { row.style.display = ''; } else { row.style.display = 'none'; }
            });
            ...
            mobileCards.forEach(card => {
                ...
                if (matchesSearch && matchesMention) { card.style.display = ''; } else { card.style.display = 'none'; }
            });
        }

        document.getElementById('searchInput').addEventListener('input', filterCourses);
        if (document.getElementById('mentionFilter')) { document.getElementById('mentionFilter').addEventListener('change', filterCourses); }
        document.addEventListener('DOMContentLoaded', function() { filterCourses(); });
        ...
        if (toggle) toggle.addEventListener('click', function() { document.querySelector('.sidebar').classList.toggle('active'); });

        document.addEventListener('click', function(event) {
            const sidebar = document.querySelector('.sidebar');
            const toggleBtn = document.getElementById('sidebarToggle');
            if (sidebar && toggleBtn && window.innerWidth < 768 && !sidebar.contains(event.target) && !toggleBtn.contains(event.target) && sidebar.classList.contains('active')) {
...
<?php /**PATH D:\PROJET REGISTRAIRE\registrarClone\registrar\resources\views/teacher/courses/list.blade.php ENDPATH**/ ?>